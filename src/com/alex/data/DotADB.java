			...
			try {
				...
				document = DocumentHelper
						.parseText(getJsonString("http://api.steampowered.com/IDOTA2Match_570/GetMatchHistory/v001?"
								+ "key=188DE5A4011C5BBD197C5ECF3EABD15F&format=XML&matches_requested=1&account_id=175151266"));
				...
				int total_num = Integer.parseInt(document.getRootElement().elementText("total_results"));
				...
				while (true) {
					document = DocumentHelper
							.parseText(getJsonString("http://api.steampowered.com/IDOTA2Match_570/GetMatchHistory/v001?"
									+ "key=188DE5A4011C5BBD197C5ECF3EABD15F&format=XML&matches_requested=50&account_id=175151266&start_at_match_id="
									+ start_at_match_id));
					...
					for (int j = 1; j < list.size(); j++) {
						...
						document = DocumentHelper.parseText(getJsonString("https://api.steampowered.com/IDOTA2Match_570/GetMatchDetails/v001?"
				+ "key=188DE5A4011C5BBD197C5ECF3EABD15F&format=XML&match_id="+match_id));
						String win = document.getRootElement().elementText("radiant_win");
						playerList = document.getRootElement().element("players").elements("player");
						for(Object o : playerList){
							
							Element elementPlayer = (Element)o;
							Log.i("http", elementPlayer.elementText("account_id")+" "+account_id);
							if(elementPlayer.elementText("account_id").equals(account_id)){
								
								int slot = Integer.parseInt(elementPlayer.elementText("player_slot"));
								boolean radiant_win;
							    if(win.equals("true")){
							    	radiant_win = true;
							    }else {
								...
								heal = elementPlayer.elementText("hero_healing");
								damage = elementPlayer.elementText("hero_damage");
								tower_damage = elementPlayer.elementText("tower_damage");
								hits = elementPlayer.elementText("last_hits");
								denies = elementPlayer.elementText("denies");
								level = elementPlayer.elementText("level");
								gold_per_min = elementPlayer.elementText("gold_per_min");
								xp_per_min = elementPlayer.elementText("xp_per_min");
								getReadableDatabase().execSQL("insert into match values(null,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",new String[]{
										match_id,start_time,has_won,hero_id,kills,death,assiant,damage,tower_damage,hits,denies,level,gold_per_min,
										xp_per_min
								});
								num++;
								Message msg = activity.getHandler().obtainMessage();
								msg.what = 911;
								msg.obj = (Integer)((int)((double)num/(double)total_num*100));
